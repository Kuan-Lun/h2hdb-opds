...
不支援 OR 或排除條件；不要把 `OR` 或 `NOT` 當成運算子。
...
| 帶有中文標籤的作品 | `tag:language:chinese` |
| 同時帶有語言與作者標籤 | `tag:language:chinese tag:artist:alice` |
| 標題含「不知火」、帶有中文標籤，且為 40 到 200 頁 | `title:不知火 tag:language:chinese pages:40..200` |
...
| `tag:命名空間:值` | 精確比對來源標籤，例如 `tag:artist:alice` |
| `tag:命名空間:"含空白的值"` | 例如 `tag:artist:"a  b"`，中間的兩個空白必須一致 |

欄位名稱使用小寫，例如 `title:`。多個 `title:` 條件或多個不同標籤都是 AND；
...
命名空間也可以加雙引號，例如 `tag:"artist":"alice"`。
雙引號內可用 `\"` 表示引號、`\\` 表示反斜線。
...
請在搜尋框輸入多個 `tag:`。選擇該組的 `All` 會清除該組篩選，
`More` 則會列出更多可選值。
...
- 不支援 `author:`、`language:` 等其他欄位名稱；可改用篩選選單或來源標籤。
- 日期必須存在，日期與頁數區間不能倒置。
...
curl --get 'http://127.0.0.1:8000/opds/v1.2/search' \
  --data-urlencode 'q=title:不知火 tag:language:chinese pages:40..200'
...
| 參數 | 用法 |
| --- | --- |
| `language=值` | 精確比對作品語言；與 `tag:language:chinese` 這類來源標籤是不同條件 |
