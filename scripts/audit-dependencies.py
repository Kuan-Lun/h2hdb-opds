...
def _audit_node(name: str, declared: str) -> dict[str, Any]:
    ...
    if not isinstance(latest, str) or not latest:
        raise ValueError(f"npm returned an invalid latest version for {name}")
    compatible_completed = subprocess.run(
        ("npm", "view", f"{name}@{declared}", "version", "--json"),
        check=True,
        stdout=subprocess.PIPE,
        text=True,
    )
    compatible = json.loads(compatible_completed.stdout)
    compatible_versions = compatible if isinstance(compatible, list) else [compatible]
    if not compatible_versions or not all(
        isinstance(version, str) and version for version in compatible_versions
    ):
        raise ValueError(f"npm returned invalid compatible versions for {name}")
    return {
        "declared": declared,
        "latest": latest,
        "latest_satisfies": latest in compatible_versions,
        "name": name,
    }
