...
revisions. Production startup uses `open_database()` for the exact epoch-2
`READY` audit. An injected `CatalogReader` is a caller-supplied,
already-initialized boundary and is used directly. No startup path may call
`migrate()`.
